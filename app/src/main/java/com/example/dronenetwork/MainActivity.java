...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String address =  getCurrentAddress(latitude, longitude);   //현재 위치
        showText = "현재위치\n " + address + "\n위도 : " + latitude + "\n경도 : " + longitude;
        length = showText.length();
        //textview_address.setText(showText); //텍스트창에 주소 위도 경도 표시
        if(isEnd) {
            mHandler.post(textShow);
            isEnd = false;
        }
        else{
            isEnd = true;
    ...
    Runnable textShow = new Runnable() {
        @Override
        public void run() {
            try {
                if (!isEnd || index < length - 2) {
                    Log.i("DEBUG", "strR inde = " + index + ", length = "
                            + length);
                    String flowString = showText.substring(0, ++index);
                    textview_address.setText(flowString);
                    mHandler.postDelayed(textShow, 20);
                } else {
                    isEnd = true;
                    index = 0;
                }
